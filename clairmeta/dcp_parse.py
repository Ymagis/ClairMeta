...
def generic_parse(path, root_name, force_list=(), namespaces=DCP_SETTINGS["xmlns"]):
    """Parse an XML and returns a Python Dictionary"""
    try:
        res_dict = parse_xml(path, namespaces=namespaces, force_list=force_list)
...
def cpl_parse(path):
    """Parse DCP CPL"""
    cpl = generic_parse(
        path, "CompositionPlaylist", ("Reel", "ExtensionMetadata", "PropertyList")
    )
...
def cpl_reels_parse(cpl_node):
    ...
    for pos, in_reel in enumerate(in_reels, 1):
                ...
                if isinstance(asset, list):
                    raise ProbeException(
                        "Duplicated {} asset in CPL {}, Reel {}".format(
                            key, cpl_node.get("ContentTitleText", ""), pos
                        )
                    )
        ...
        if "Metadata" in out_reel["Assets"]:
            ...
            exts = meta.get("ExtensionMetadataList", {}).get("ExtensionMetadata", [])
            for ext in exts:
...
def kdm_parse(path):
    ...
    keys = {}
    counter_mapping = {"MDIK": 0, "MDAK": 0, "MDSK": 0, "MDEK": 0}
    ...
    if in_dict and "DCinemaSecurityMessage" in in_dict:
        ...
        if (
            "KDMRequiredExtensions" in req_ext
            and req_ext["KDMRequiredExtensions"]["AuthorizedDeviceInfo"]
        ):
            root = req_ext["KDMRequiredExtensions"]

            keys_pub = root["KeyIdList"]["TypedKeyId"]
            keys_priv = in_dict["DCinemaSecurityMessage"]["AuthenticatedPrivate"][
                "EncryptedKey"
            ]

            for pub, priv in zip(keys_pub, keys_priv):
                keys[pub["KeyId"]] = {"Cipher": priv["CipherData"]["CipherValue"]}
