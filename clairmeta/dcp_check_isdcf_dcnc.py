...
class Checker(CheckerBase):
    ...
    def run_checks(self):
        for source in self.dcp._list_cpl:
            ...
            self.run_check(self.check_dcnc_compliance, source, stack=asset_stack)
    ...
    def check_dcnc_field_dimension(self, playlist, fields):
        ...
        if is_3D and dimension_content == "":
            self.error(
                "Content Type should specify 2D version or " "3D version for 3D Movie"
            )
    ...
    def check_dcnc_field_aspect_ratio(self, playlist, fields):
        ...
        if is_trailer and iar_qualifier != "":
            self.error(
                "Trailer content should not contain " "ImageAspectRatio qualifier"
            )
    ...
    def check_dcnc_field_package_type(self, playlist, fields):
        ...
        if pkg_type == "OV" and pkg_version != "":
            self.error(
                "OV Package can't include a version number " "in the package type field"
            )
    ...
    def check_dcnc_field_claim_dimension(self, playlist, fields):
        ...
        is_stereo_map = {"2D": False, "3D": True}
        if (
            dimension
            and cpl_stereo != "Mixed"
            and is_stereo_map[dimension] != cpl_stereo
        ):
            self.error(
    ...
    def check_dcnc_field_claim_audio(self, playlist, fields):
        ...
        sounds = list(
            list_cpl_assets(playlist, filters=["Sound"], required_keys=["Probe"])
        )
    ...
    def check_dcnc_field_claim_dolbyvision(self, playlist, fields):
        ...
        if dvi_dcnc and not dvi_cpl:
            self.error(
                "ContentTitle claims DolbyVision but CPL " "miss required metadata"
            )
        elif not dvi_dcnc and dvi_cpl:
            self.error(
                "CPL imply DolbyVision but ContentTitle miss " "DVis ContentType field"
            )
    ...
    def check_dcnc_field_claim_eclaircolor(self, playlist, fields):
        ...
        if ec_dcnc and not ec_cpl:
            self.error(
                "ContentTitle claims EclairColor but CPL " "miss required metadata"
            )
        elif not ec_dcnc and ec_cpl:
            self.error(
                "CPL imply EclairColor but ContentTitle miss " "EC ContentType field"
            )
