...
def execute_command(cmd_args):
    ...
    get_log().debug(
        "Executed command with return code ({})\n{}".format(
...
def probe_mxf_clean(in_meta):
    ...
    if is_picture:
        out_meta["Resolution"] = "{}x{}".format(
            out_meta["StoredWidth"], out_meta["StoredHeight"]
        )
...
def stat_mxf_audio(path, channels, entry_point, duration):
    ...
    args = [
        ...
        "-f",
        str(entry_point),  # Starting frame number
        "-d",
        str(duration),  # Number of frames to process
    ]
    ...
    with unwrap_mxf(path, prefix=prefix, args=args) as folder:
        ...
        sox_args = (
            [SOX_CMD, "-t", "wavpcm", "-M"]  # File type of audio
            + wav_list
            + [  # Merge multiple input files
                "-n",
                "stats",  # Use the `null' file handler
            ]
        )
