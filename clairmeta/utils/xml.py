...
def post_parse_node(path, key, value, ns_prefix=" "):
    """ Xmltodict postprocessor function.
        ...
        Rules applied :
         - XML namespace are either replaced by alias we specify or leave as is
           ...
           namespace<ns_prefix>key, so we extract the key to have clean dict
    ...
    if not is_attribute and len(tag_split) == 2:
        ...
        found = sum(p.startswith(ns + ns_prefix) for p in paths_prefix)
    ...
    try:
        with open(xml_path, encoding="utf-8-sig") as file:
            ...
            xml_dict = xmltodict.parse(
                ...
                postprocessor=partial(post_parse_node, ns_prefix=ns_prefix),
                dict_constructor=dict,
                namespace_separator=ns_prefix
            )
