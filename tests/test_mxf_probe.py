...
class TestAssetProbe(unittest.TestCase):
    def get_path(self, name):
        file_path = os.path.join(os.path.dirname(__file__), "resources", "MXF", name)

        return file_path

    def test_video_iop(self):
        metadata = probe_mxf(self.get_path("picture_2D_iop.mxf"), stereoscopic=False)
        self.assertTrue(metadata["EditRate"] == 24)
        self.assertTrue(metadata["LabelSetType"] == "MXFInterop")

    def test_video_smpte(self):
        metadata = probe_mxf(self.get_path("picture_2D_smpte.mxf"), stereoscopic=False)
        self.assertTrue(metadata["EditRate"] == 24)
